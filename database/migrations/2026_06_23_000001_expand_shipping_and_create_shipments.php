...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('shipping_methods', function (Blueprint $table) {
            if (! Schema::hasColumn('shipping_methods', 'per_kg_cost')) {
                $table->decimal('per_kg_cost', 15, 2)->default(0)->after('base_cost');
            }

            if (! Schema::hasColumn('shipping_methods', 'min_order_total')) {
                $table->decimal('min_order_total', 15, 2)->nullable()->after('per_kg_cost');
            }

            if (! Schema::hasColumn('shipping_methods', 'max_order_total')) {
                $table->decimal('max_order_total', 15, 2)->nullable()->after('min_order_total');
            }

            if (! Schema::hasColumn('shipping_methods', 'min_weight')) {
        ...
        Schema::table('orders', function (Blueprint $table) {
            ...
            if (! Schema::hasColumn('orders', 'shipping_min_delivery_days')) {
                $table->unsignedInteger('shipping_min_delivery_days')->nullable()->after('shipping_weight');
            }

            if (! Schema::hasColumn('orders', 'shipping_max_delivery_days')) {
    ...
    public function down(): void
    {
        ...
        Schema::table('orders', function (Blueprint $table) {
            ...
            $orderColumns = array_values(array_filter([
                Schema::hasColumn('orders', 'shipping_weight') ? 'shipping_weight' : null,
                Schema::hasColumn('orders', 'shipping_min_delivery_days') ? 'shipping_min_delivery_days' : null,
                Schema::hasColumn('orders', 'shipping_max_delivery_days') ? 'shipping_max_delivery_days' : null,
            ]));
        ...
        Schema::table('shipping_methods', function (Blueprint $table) {
            $shippingColumns = array_values(array_filter([
                Schema::hasColumn('shipping_methods', 'per_kg_cost') ? 'per_kg_cost' : null,
                Schema::hasColumn('shipping_methods', 'min_order_total') ? 'min_order_total' : null,
                Schema::hasColumn('shipping_methods', 'max_order_total') ? 'max_order_total' : null,
                Schema::hasColumn('shipping_methods', 'min_weight') ? 'min_weight' : null,
                Schema::hasColumn('shipping_methods', 'max_weight') ? 'max_weight' : null,
            ]));

            if ($shippingColumns !== []) {
                $table->dropColumn($shippingColumns);
            }
        });
    }
};
